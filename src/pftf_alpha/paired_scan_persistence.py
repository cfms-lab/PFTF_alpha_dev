"""Paired-scan local surface persistence and frozen Phase-15 audit."""
...
CALIBRATION_A_SEED = 21800804
CALIBRATION_B_SEED = 21900804
FINAL_HELD_OUT_SEED = 22000804
REPLICATE_SEED_OFFSET = 400000009


@dataclass(frozen=True)
class PairedScanPersistenceConfig:
    """Frozen observed paired-scan representation for Phase 15."""
...
@dataclass(frozen=True)
class PairedScanPersistenceScores:
...
@dataclass(frozen=True)
class PairedScanPersistenceEvidence:
...
@dataclass(frozen=True)
class PairedScanRawCase:
    stress: SensorStress
    point_count: int
    repeat: int
    seed: int
    replicate_seed: int
    evidence: PairedScanPersistenceEvidence
    endpoint: GeometryTopologyHarmEndpoint
    unguarded_decision: SamplingGateDecision


@dataclass(frozen=True)
class PairedScanCaseResult:
    stress: SensorStress
    point_count: int
    repeat: int
    seed: int
...
@dataclass(frozen=True)
class PairedScanPanelResult:
    panel_role: str
    seed: int
    rectangle: InfluenceRectangle | None
    cases: tuple[PairedScanCaseResult, ...]
    case_count: int
...
@dataclass(frozen=True)
class PairedScanPersistenceResult:
    artifact_schema: str
    role: str
    information_boundary: str
    ...
    calibration_a: PairedScanPanelResult
    calibration_b: PairedScanPanelResult
    final_held_out: PairedScanPanelResult | None
    phase15_supported: bool
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "phase15_supported": self.phase15_supported,
...
def paired_scan_persistence_scores(
    primary_points: FloatArray,
    primary_labels: IntArray,
    replicate_points: FloatArray,
    replicate_labels: IntArray,
    config: PairedScanPersistenceConfig | None = None,
) -> PairedScanPersistenceScores:
    """Score primary points against independent replicate local surfaces."""

    selected = PairedScanPersistenceConfig() if config is None else config
        ...
        for local_index, ordered_neighbors in enumerate(neighbor_rows):
            ...
            for scale_index, neighbor_count in enumerate(available_counts):
                ...
                predicted_height = float(
                    _quadratic_design(point_tangent)[0] @ coefficients
                )
    ...
    return PairedScanPersistenceScores(
...
def estimate_paired_scan_persistence(
    primary_points: FloatArray,
    primary_labels: IntArray,
    replicate_points: FloatArray,
    replicate_labels: IntArray,
    config: PairedScanPersistenceConfig | None = None,
) -> PairedScanPersistenceEvidence:
    selected = PairedScanPersistenceConfig() if config is None else config
    ...
    return PairedScanPersistenceEvidence(
        information_boundary=(
            "primary_and_replicate_coordinates_with_independently_inferred_layers"
        ),
...
def _raw_panel(
    *,
    ...
    harm_config = LocalSurfaceConsensusConfig(
        harmful_distance_fraction=persistence_config.harmful_distance_fraction
    )
    rows: list[PairedScanRawCase] = []
    for case_row in base_result.cases:
        ...
        replicate_seed = case_row.seed + persistence_config.replicate_seed_offset
        ...
        evidence = estimate_paired_scan_persistence(
            primary.points,
...
def _features(row: PairedScanRawCase) -> tuple[float, float]:
    return (
...
def _feature_cohort(rows: tuple[PairedScanRawCase, ...]) -> InfluenceFeatureCohort:
    harmful = tuple(
        _features(row)
        for row in rows
...
def _materialize_panel(
    raw_rows: tuple[PairedScanRawCase, ...],
    *,
    panel_role: str,
    seed: int,
    rectangle: InfluenceRectangle | None,
    full_protocol: bool,
) -> PairedScanPanelResult:
    rows: list[PairedScanCaseResult] = []
    for raw in raw_rows:
...
def evaluate_paired_scan_persistence(
    *,
    ...
    persistence_config: PairedScanPersistenceConfig | None = None,
) -> PairedScanPersistenceResult:
    ...
    full_protocol = bool(
        ...
        and selected_persistence.replicate_seed_offset == REPLICATE_SEED_OFFSET
    )
    common = {
        ...
        "base_gate_config": base_gate_config,
        "shared_trend_config": shared_trend_config,
        "persistence_config": selected_persistence,
    }
    ...
    final_held_out: PairedScanPanelResult | None = None
    ...
    return PairedScanPersistenceResult(
        artifact_schema="pftf_alpha_paired_scan_persistence_phase15/v1",
        role="independent_repeat_local_surface_persistence_guard",
        information_boundary=(
            "route uses primary and independently repeated coordinates with "
            "inferred layers; stress, source labels, and references are evaluation-only"
        ),
        frozen_predecessor="phase14_seeds_21600804_21700804_negative",
