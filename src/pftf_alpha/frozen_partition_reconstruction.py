"""Frozen full-primary partition reconstruction audit for Phase 26."""
...
PRIOR_PANEL_BASE_SEEDS = tuple(index * 100000 + 804 for index in range(203, 257))
...
class CaseSeedDisjointnessAudit:
    case_seed_formula: str
    prior_base_seed_start: int
    prior_base_seed_end: int
...
@dataclass(frozen=True)
class FrozenPartitionCaseResult:
...
@dataclass(frozen=True)
class FrozenPartitionProfileSummary:
    profile: MatchedPairStressProfile
    case_count: int
...
@dataclass(frozen=True)
class FrozenPartitionPanel:
    panel_role: str
    seed: int
    cases: tuple[FrozenPartitionCaseResult, ...]
    profile_summaries: tuple[FrozenPartitionProfileSummary, ...]
    case_count: int
    unguarded_harmful_outlier_false_safe_count: int
    guarded_harmful_outlier_false_safe_count: int
    focus_unguarded_safe_accept_count: int
    ...
    all_stress_unguarded_safe_accept_count: int
    all_stress_guarded_safe_accept_count: int
    frozen_partition_case_count: int
    introduced_routed_endpoint_harm_accept_count: int
    full_protocol: bool
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "frozen_partition_case_count": self.frozen_partition_case_count,
...
@dataclass(frozen=True)
class FrozenPartitionReconstructionResult:
    artifact_schema: str
    role: str
    ...
    case_seed_disjointness: CaseSeedDisjointnessAudit
    ...
    phase26_supported: bool
    frozen_partition_reconstruction_synthetic_supported: bool
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "phase26_supported": self.phase26_supported,
            "frozen_partition_reconstruction_synthetic_supported": (
                self.frozen_partition_reconstruction_synthetic_supported
            ),
...
def _panel_case_seeds(
    ...
    *,
    ...
    return frozenset(
        ...
        + stress_index * 100003
        + repeat * 10007
        for count_index, _ in enumerate(point_counts)
        for stress_index, _ in enumerate(stresses)
        for repeat in range(repeats)
    )


def audit_case_seed_disjointness(
    validation_a_seed: int,
    validation_b_seed: int,
    final_held_out_seed: int,
    *,
    ...
    prior = frozenset().union(
        *(
            ...
            for seed in PRIOR_PANEL_BASE_SEEDS
        )
    )
    panels = tuple(
        _panel_case_seeds(
    ...
    return CaseSeedDisjointnessAudit(
        case_seed_formula=(
            "base + count_index*1000003 + stress_index*100003 + repeat*10007"
        ),
        prior_base_seed_start=PRIOR_PANEL_BASE_SEEDS[0],
        prior_base_seed_end=PRIOR_PANEL_BASE_SEEDS[-1],
...
def _routed_endpoint(
    ...
    return raw.frozen_partition_endpoint if raw.missing_pair_count else raw.endpoint
...
def _materialize_case(
    ...
    return FrozenPartitionCaseResult(
        profile=raw.profile,
        stress=raw.stress,
        ...
        frozen_partition_applied=bool(raw.missing_pair_count),
        signature=signature,
        model_score=score,
...
def _profile_summary(
    rows: Sequence[FrozenPartitionCaseResult],
    profile: MatchedPairStressProfile,
    *,
    full_protocol: bool,
    model_valid: bool,
) -> FrozenPartitionProfileSummary:
    ...
    return FrozenPartitionProfileSummary(
        profile=profile,
...
def _materialize_panel(
    ...
    *,
    ...
    return FrozenPartitionPanel(
        ...
        frozen_partition_case_count=sum(
            row.frozen_partition_applied for row in rows
        ),
...
def evaluate_frozen_partition_reconstruction(
    *,
...
) -> FrozenPartitionReconstructionResult:
    ...
    seed_audit = audit_case_seed_disjointness(
        *fresh_seeds,
    ...
    return FrozenPartitionReconstructionResult(
        artifact_schema="pftf_alpha_frozen_partition_reconstruction_phase26/v1",
        role="synthetic_frozen_partition_reconstruction_guard_audit",
        information_boundary=(
            "route uses full primary coordinates and their observed shared-trend "
            "partition, presented retained-pair IDs and repeat coordinates, "
            "upstream candidate decision, and frozen observed guard; source "
            "labels, clean references, and harm endpoints are evaluation-only"
        ),
        frozen_predecessor=(
            "phase25_subset_reclustering_introduced_clean_cross_layer_faces"
        ),
        ...
        phase26_supported=supported,
        frozen_partition_reconstruction_synthetic_supported=supported,
