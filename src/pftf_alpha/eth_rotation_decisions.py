"""Materialize Phase-38 rotation decisions before ETH labels are opened."""
...
from .fresh_external_protocol import EXPECTED_PAIR_COUNT
...
class ETHBlindRotationDecision:
    source_index: int
    target_index: int
...
@dataclass(frozen=True)
class ETHBlindRotationDecisionArtifact:
    artifact_schema: str
    role: str
    ...
    decisions: tuple[ETHBlindRotationDecision, ...]
    accepted_count: int
    rejected_count: int
...
def _load_predictions(path: Path) -> Mapping[str, object]:
    if _sha256(path) != EXPECTED_PREDICTION_SHA256:
        raise ValueError("Phase-38 prediction artifact SHA-256 mismatch")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("Phase-38 predictions must be a JSON object")
    expected = {
    ...
    for key, value in expected.items():
        if payload.get(key) != value:
            raise ValueError(f"Phase-38 prediction protocol mismatch: {key}")
    return payload


def materialize_eth_rotation_decisions(
    prediction_path: str | Path,
) -> ETHBlindRotationDecisionArtifact:
    path = Path(prediction_path)
    payload = _load_predictions(path)
    raw_predictions = payload.get("predictions")
    if not isinstance(raw_predictions, list):
        raise ValueError("Phase-38 predictions are missing")
    ...
    accepted = sum(decision.guarded_accept for decision in decisions)
    return ETHBlindRotationDecisionArtifact(
        artifact_schema="pftf_alpha_eth_rotation_decisions_phase38/v1",
        ...
        label_boundary=(
            "this program accepts only the frozen prediction JSON and has no "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = materialize_eth_rotation_decisions(args.predictions)
